...
const AccordionPath = () => {
  ...
  const imgSrc = paths.find((s) => s.id === open)?.imgSrc;
  return (
    <section className="px-8 py-12 bg-white">
      <div className="w-full max-w-5xl mx-auto grid gap-8 grid-cols-1 sm:grid-cols-[1fr_350px]">
        <div>
          <h3 className="text-4xl font-bold mb-8 text-violet-600">Paths</h3>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0 }}
            key={imgSrc}
            className="bg-slate-300 rounded-2xl aspect-[4/3] sm:aspect-auto"
            style={{
...
const Solution = ({
  ...
  return (
    <div
      ...
      className="p-0.5 rounded-sm relative overflow-hidden cursor-pointer"
    >
      <motion.div
        initial={false}
        animate={{
