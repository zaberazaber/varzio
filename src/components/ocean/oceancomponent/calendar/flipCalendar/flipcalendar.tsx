import React, {
  ...
  export const FlipCalendarExample = () => {
    return (
      <div className="grid place-content-center bg-rose-50 px-4 py-24 md:flex-row">
        <FlipCalendar />
      </div>
    );
  };
  ...
  export const FlipCalendar = () => {
    ...
    return (
      <div className="relative flex flex-col items-center text-rose-950">
        <CalendarDisplay
          index={index}
          date={date}
          visible={visible}
  ...
  const CalendarDisplay = ({
    ...
    return (
      <div className="w-fit overflow-hidden rounded-xl border-2 border-rose-500 bg-rose-500">
        <div className="flex items-center justify-between px-1.5 py-0.5">
          ...
          <button
            ...
            className="text-white transition-colors hover:text-white-200"
          >
        ...
        <div className="relative z-0 h-36 w-52 shrink-0">
          <AnimatePresence mode="sync">
            ...
            >
              <div className="grid h-full w-full place-content-center rounded-lg bg-rose-400 text-6xl">
                {format(date, "do")}
              </div>
            </motion.div>
            <motion.div
  ...
  const DatePicker = (props: DatePickerProps) => {
    ...
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="-right-4 top-0 mt-4 w-fit rounded-lg border border-rose-500 bg-white p-3 md:absolute md:mt-0 md:translate-x-full"
      >
        ...
        <div key={`${calendar.month}${calendar.year}`} className="w-52">
          ...
          {calendar.weeks.map((week, weekIndex) =>
            week.map((dateObj, index) => {
              ...
              return (
                <button
                  className={`inline-block w-[calc(100%_/_7)] rounded text-sm transition-colors ${selected ? "bg-rose-500 text-white" : "bg-transparent"}`}
                  key={key}
